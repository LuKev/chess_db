...
33. Web board rendering now uses a shared `ChessPiece` image component (`apps/web/components/ChessPiece.tsx`) backed by vendored open-source piece assets in `apps/web/public/pieces/` from `chessboard-element@1.2.0` (MIT). Included sets are `alpha`, `uscf`, and `wikipedia`; current default is `alpha`.
