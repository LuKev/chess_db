...
27. Web UI convention: prefer lighter/smaller buttons in `apps/web/app/globals.css`; `/games` filters are reactive (no Apply button) and reset pagination/selection on change, while display preferences (default 50/page, font size, column width, visible columns) persist in browser storage. The app-shell left sidebar can also be collapsed from the header and its state persists in browser storage.
