...
32. Active product direction question: user pushed back on per-ply persistence in `game_positions`; potential follow-up is to make position indexing optional (feature flag/background/offline index) or redesign schema to avoid direct one-row-per-position writes on ingest.
